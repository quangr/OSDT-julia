...
struct Data
    ...
    function Data(X::BitMatrix, y::BitArray)
        ...
        new(X, y, numdata, minority, sum(minority))
    end
end
struct Fitter
    data::Data
...
function EquivalentPointsBound(fitter::Fitter, tree::Tree)::Bool
    if (fitter.logger.B_c <= fitter.data.numminority / fitter.data.numdata + fitter.lamb * tree.node)
        return (false)
    end
    tobesplit = filter(x -> x.can_split, tree.leaves)
    minority = fitter.data.minority
    for i in tobesplit
        minority .&= i.captured
    end
    if (fitter.logger.B_c <= tree.penalty + sum(minority) / fitter.data.numdata)
        return (false)
    end
    return (true)
end
...
function pushtofitter(fitter::Fitter, tree::Tree)
    if (EquivalentPointsBound(fitter, tree))
...
function get_splitable_leaves!(fitter::Fitter, t::Tree, nrule::Int)
    ...
    if num_tosplit != 0
        ...
        if t.penalty < fitter.logger.B_c
            fitter.logger.B_c = t.penalty
            fitter.logger.BestTree = t
        end
    end
end
...
function Tree(leaves::Vector{Leaf}, fitter::Fitter, node::Int)
    not_split = filter(x -> !x.can_split, leaves)
    ac = 0
    if (!isempty(not_split))
        ac = map(not_split) do x
                 x.num_predicted
